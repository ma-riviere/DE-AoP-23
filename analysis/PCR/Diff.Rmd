...
diff_data_cond |> 
  ...
  group_walk(\(d, g) {
    ...
    save_png(
      ...
      Diff_fig_path = "Diff", width = 1 + n.cols * 2, height = 1 + n.rows * 4
    )
  }, .keep = TRUE
)
```
  ...
  |> group_walk(
    \(d, g) {
      ...
      make_vertical_fold_timeline(d, facet_rows = "Pathway_family", trans = "log", colors = colors_fold, title = g$Layer_Family[[1]], size_boost = 1.5) |> # 2
        save_png(
          glue("[{Diff_fig_path}] Timeline of Fold Change by [Pathway_family] - {g$Layer_Family[[1]]}"),
          Diff_fig_path = "Diff", width = width, height = height
        )
    }
  )
)
