...
load_pcr_data <- function(target, reprocess = FALSE, max_cq_clean = 33, refit = FALSE, model) {
    ...
    if (reprocess) {
        ...
        ## Raw data
        res$raw <- (map(set_names(excel_sheets(path)), \(x) read_excel(path, sheet = x)) |>
        ...
        if (refit) {
            ...
            compute_fold_change <- function(mod) {
                return(
                    get_data(mod) |>
                        ...
                        pivot_wider(names_from = Condition, values_from = DCq, values_fn = \(x) {
                            mean(x, na.rm = TRUE)
                        }) |>
            ...
            res$models <- (res$clean |>
                ...
                map_dfr(
                    \(d) {
                        suppressMessages({
                            summarize(d, Mod = list(model(pick(everything()))), .by = c(Stage, Layer, Gene))
                        })
                    },
...
load_calb_data <- function(path = configs$data$IHC$calb) {
    ...
    res$raw <- (map(set_names(excel_sheets(path)), \(sheet) read_excel(path, sheet)) |>
        list_rbind(names_to = "Stage") |>
        arrange(Sample) |>
        mutate(
