...
make_circlize_plot <- function(dat, resolution = 15000, save_to = NULL) {
    ...
    crossed_layer_order <- (crossing(Stage = stage_list, Layer = layer_list) |>
    ...
    df_layer <- (tibble(Sector = rownames(mat_chord)) |>
        mutate(N_Connection = rowSums(mat_chord)) |>
        filter(N_Connection > 0))

    df_pathway <- (tibble(Sector = colnames(mat_chord)) |>
    ...
    df_extra <- (bind_rows(df_layer, df_pathway) |>
        mutate(
    ...
    if (!is.null(save_to)) {
        png(save_to, width = resolution, height = resolution)
    }
    ...
    circos.track(
        ...
        panel.fun = \(x, y) {
            if (str_detect(CELL_META$sector.index, "P[0-9]+_")) {
                ...
                xpos <- seq(
                    from = CELL_META$cell.xlim[1] + 0.05,
                    to = CELL_META$cell.xlim[2] - 0.05,
                    length.out = nrow(genes_barplot)
                )
...
make_signif_boxplot <- function(
    ...
    if (is.null(resp_name)) {
        resp_name <- get_response_name(resp, "IHC")
    }
    ...
    plot <- (ggplot(dat, aes(x = .data[[xaxis]], y = .data[[resp]], color = .data[[xaxis]], fill = .data[[xaxis]])) +
        geom_boxplot(outlier.alpha = 0, linewidth = 1.1, fill = NA) +
        stat_summary(
            fun = mean,
            geom = "errorbar",
            ...
            if (!is.null(cluster)) {
                geom_jitter(
                    data = \(x) {
                        x |>
                            group_by(across(any_of(c(xaxis, facet)))) |>
                            group_modify(\(d, g) slice_sample(d, n = min(nrow(d), 50))) |>
                            ungroup()
                    },
                    size = 1.5,
                    width = 0.1,
...
make_signif_boxplot_inter <- function(
    ...
    if (tolower(scale) %in% c("response", "resp")) {
        emmeans <- regrid(emmeans, transform = "response")
    }
    ...
    p_data_interactions <- contrasts_interactions |>
        mutate(
            x1 = 0.5 *
                ((match(F1, levels(dat[[pred2]])) - 1) *
                    length(unique(dat[[pred1]])) +
                    match(X1, levels(dat[[pred1]])) +
                    (match(F1, levels(dat[[pred2]])) - 1) * length(unique(dat[[pred1]])) +
                    match(X2, levels(dat[[pred1]]))),
            x2 = 0.5 *
                ((match(F2, levels(dat[[pred2]])) - 1) *
                    length(unique(dat[[pred1]])) +
                    match(X1, levels(dat[[pred1]])) +
                    (match(F2, levels(dat[[pred2]])) - 1) * length(unique(dat[[pred1]])) +
                    match(X2, levels(dat[[pred1]]))),
            p.signif = label_pval(p.value)
        ) |>
        arrange(x.diff := abs(x2 - x1)) |>
...
ppc_plots <- function(mod, simulations, term = "Condition", type = "fixed", is_count = NULL, max_cols_per_plot = 3) {
    ...
    if (is.null(is_count)) {
        is_count <- insight::get_family(mod)$family |> str_detect("binom|poiss")
    }
...
make_suburst_plot <- function(dat, layers, tooltips = NULL, colors = NULL, plot_options = list()) {
    if (!is.null(colors)) {
        colors <- set_names(colors, \(x) x |> str_replace_all(" ", "\n") |> str_replace_all("and\n", "and "))
    }
                            ...
                            transmute(
                                ...
                                hovertext = pmap_chr(
                                    ...
                                    \(...) {
                                        str_c(
                                            names(c(...)),
                                            map_if(list(...), is.numeric, \(x) round(x, 3)) |>
    ...
    params <- map_dfr(seq_along(layers), \(x) .make_sunburst_data(dat, layers[1:x])) |>
...
make_diag_reactable <- function(dat) {
    make_diagnostic_plots <- function(mod) {
        ...
        if (!is.null(find_random(mod))) {
            diag_list <- c("normality", "homogeneity", "qq", "reqq", "pp_check")
        }
