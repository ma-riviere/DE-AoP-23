...
make_boxplot_panel <- function(data, n_cols = NULL, n_rows = 1, display_N = TRUE, p.name = "p.value", display_labels = FALSE, invert_DCq = TRUE, color_by = "Condition") {
...
make_heatmap <- function(data, xaxis, facet) {
  ...
  return(
    ...
    + geom_tile(data = \(d) filter(d, Fold < 1), aes(fill = Fold), colour = "white")
    + geom_text(aes(label = paste(round(Fold, 2), stars.pval(p.value), sep = " ")), size = 3, colour = "white", fontface = "bold", check_overlap = TRUE)
