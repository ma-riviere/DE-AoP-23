...
sunburst_pcr_colors <- list(
    ...
    Whole = "#321359",
    # Expression
    Downregulated = "#f02b00",
    Upregulated = "#01944b",
    # ND pathway families
    `Proliferation and Repair` = "#eb4f2d",
    `Migration and Response` = "#83c108",
    `Cellular Differentiation` = "#f5a402",
    `Cell Communication` = "#2cc969",
    # ND pathways
    `Fate Mapping` = "#f02b00",
    Survival = "#faa491",
    `Myelin Sheath` = "#f59002",
    `Neurite Growth` = "#ffda8f",
    Guidance = "#81c700",
    Motility = "#d1eb52",
    Membrane = "#01944b",
    Soluble = "#78e3a1",
    # OS pathways
    `Apoptotic Pathways` = "#FFE792",
    `Autophagy and Mitophagy` = "#E78AC3",
    `Cell Death and Protection` = "#66C2A4",
    `Inflammation Pathways` = "#FC6A4A",
    `Antioxidant Response` = "#A5D853",
    `Redox Enzymes` = "#3690C0",
    `ROS Production` = "#FEB24C"
)
...
format_gt <- function(gt_tbl) {
    gt_tbl <- gt::fmt(
        ...
        fns = \(x) {
            purrr::map_chr(x, \(v) {
                ifelse(!is.na(v) && utils::type.convert(v, as.is = TRUE) |> is.numeric(), label_pval(as.numeric(v)), v)
            })
        }
    )
    ...
    gt_tbl <- gt::fmt_number(
        ...
        decimals = 3,
        drop_trailing_zeros = TRUE # n_signif = 2
    )
