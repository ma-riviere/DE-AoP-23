...
init_project_packages <- function() {
  ...
  suppressWarnings({ load_packages(project_pkgs) })
}
...
load_packages <- function(pkgs) {
  for (pkg in pkgs) {
    ...
    if (is_installed(pkg_name)) suppressPackageStartupMessages({ require(pkg_name, character.only = TRUE, quietly = TRUE, warn.conflicts = FALSE) })
  }
}