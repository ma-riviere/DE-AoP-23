...
(ND_data$models <- ND_data$clean |>
    ...
    map_dfr(
        \(d) {
            summarize(
                d,
                Mod = pick(Condition, DCq) |> ND_model() |> list(),
...
#| eval: false
#| code-fold: true
#| code-summary: "make_circlize_plot"
#| code: !expr get_function_code("make_circlize_plot")
```
...
```{r}
ND_timelines <- (ND_data$predictions |>
    left_join(supplementary_data$gene_data$ND, join_by(Gene)) |>
    ...
    mutate(
        Stage = case_when(
            ...
            factor(
                levels = c("P4", "P8", "P8 (EGLo)", "P8 (EGLi)", "P12", "P21", "P70")
            )
    ) |>
