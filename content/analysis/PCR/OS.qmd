...
(OS_data$models <- OS_data$clean |>
    ...
    map_dfr(
        \(d) {
            summarize(
                d,
                Mod = pick(Experiment, Condition, DCq) |> OS_model() |> list(),
                .by = c(Stage, Layer, Gene)
            )
        },
...
(OS_data$predictions <- OS_data$models |>
    group_split(Stage, Layer, Gene) |>
    map_dfr(
        \(d) mutate(d, get_emmeans_data(Mod[[1]])),
...
#| eval: false
#| code-fold: true
#| code-summary: "make_suburst_plot"
#| code: !expr get_function_code("make_suburst_plot")
```
...
#| eval: false
#| code-fold: true
#| code-summary: "make_fold_timeline_plot"
#| code: !expr get_function_code("make_fold_timeline_plot")
```
    ...
    if (group[[1]][1] == "Whole") {
        dat |>
            ...
            group_map(\(d, g) {
                render_OS_timeline(
                    d,
                    str_glue("Whole Cerebellum - {first(g[[1]])}"),
                    1.5
                )
            })
    } else {
...
```{r}
OS_timelines <- (OS_data$predictions |>
    left_join(supplementary_data$gene_data$OS, join_by(Gene)) |>
    filter(p.value <= .05) |>
    mutate(
        Effect = case_when(
            str_detect(Expression, "Downregulated") &
                Effect == "Beneficial" ~ "Deleterious",
            str_detect(Expression, "Downregulated") &
                Effect == "Deleterious" ~ "Beneficial",
            .default = Effect
        )
    ) |>
    select(
