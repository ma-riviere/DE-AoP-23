...
```{r}
check_overdispersion(N_CC_mod, residual_type = "normal")
```
...
```{r}
N_CC_mod_dharma <- simulateResiduals(
    N_CC_mod,
    plot = FALSE,
    n = 300,
    seed = getOption("seed")
...
(parameters(
    ...
    effects = "fixed",
    exponentiate = should_exp(N_CC_mod),
    p_adjust = "none",
    include_info = TRUE,
    digits = 3
) |>
    fix_genpois_r2(N_CC_mod) |>
    print_html() |>
...
```{r}
Vol_PC_mod_dharma <- DHARMa::simulateResiduals(
    Vol_PC_mod,
    plot = FALSE,
    n = 300,
    seed = getOption("seed")
...
ppc_stat_plots(
    Vol_PC_mod,
    simulations = Vol_PC_mod_dharma_t,
    term = "Condition"
)
```
...
(parameters(
    ...
    component = "conditional",
    effects = "fixed",
    exponentiate = should_exp(Vol_PC_mod),
    p_adjust = "none",
    include_info = TRUE,
    digits = 3
) |>
