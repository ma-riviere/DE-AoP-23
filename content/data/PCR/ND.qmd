...
ND_data$clean |>
    left_join(
        supplementary_data$gene_data$ND |> select(Pathway_family, Pathway, Gene),
        join_by(Gene)
    ) |>
    make_suburst_plot(
            ...
            cell = \(x, idx) {
                tags$div(
                    sparkline(
                        ...
                        medianColor = "grey50",
                        lineColor = "black",
                        chartRangeMin = min(ND_data$clean$DCq),
                        chartRangeMax = max(ND_data$clean$DCq)
                    ),
                    br(),
                    sparkline(
                        ND_data_summary$DCq_IH[[idx]],
                        type = "box",
                        width = 525,
                        tooltipFormatFieldlist = c('lw', 'lq', 'med', 'uq', 'rw'),
                        boxFillColor = "#9965db",
